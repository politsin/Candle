...
void GLWidget::wheelEvent(QWheelEvent *we)
{
    if (we->angleDelta().manhattanLength() >= 120) {
        if (m_zoom > 0.1 && we->angleDelta().y() < 0) {
            m_pan.setX(m_pan.x() - (we->position().x() / width() - 0.5 + m_pan.x()) * (1 - 1 / ZOOMSTEP));
            m_pan.setY(m_pan.y() + (we->position().y() / height() - 0.5 - m_pan.y()) * (1 - 1 / ZOOMSTEP));

            m_zoom /= ZOOMSTEP;
        }
        else if (m_zoom < 10 && we->angleDelta().y() > 0)
        {
            m_pan.setX(m_pan.x() - (we->position().x() / width() - 0.5 + m_pan.x()) * (1 - ZOOMSTEP));
            m_pan.setY(m_pan.y() + (we->position().y() / height() - 0.5 - m_pan.y()) * (1 - ZOOMSTEP));
